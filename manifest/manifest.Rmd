...
# You left off in (1). lavaan doesn't seem to permit this.
# It allows to provide starting values for each coefficient
# but this is not flexible as it probably doesn't allow
# stuff like sin(x1 * 2).

# Once you finish that, you were thinking about how to
# detect the specific operation of each CMV unit. For example,
# if you have a CMV between an observed and sscore, how
# would you translate that to the function that processes this
# specific case? You were thinking of adding columns for
# observed, sscore and std_sscore and then some sort
# of combination between this to generate a string that
# identifies the operation.

# 1)
# You need to fix the parser to include  normal operations on
# a sumscore, like *, /, sin, cos, etc.. The rule is that they must
# always be separate by a + because it's a sumscore

# 2)
# All variables in sumscore must be in the data
# All variables in CMV must either in the data or the sscore definition
