...
model.syntax <-
  "#1)
   sumscore = var1 + var2 * var3

   #2)
   std(std_sumscore1) = var3*w1 + w4 + sin(20) + (var4*w2) + (var5*w3)
...
model_syntax <-"~ stflife + ppltrst"

ex1 <- medesign(model_syntax, .data, me_data)
ex1


# You left off here
# You already have a parser that works, a working
# example of correctiong for several observed variables for
# their CMV and the sumscore creation function.

# Next is that if any of the sumscores are used in the CMV, you need
...
# You left off trying to adjusted the quality of the sscore
# but apparently you exclude some variables from the me_data
# above.
...
# TODO: Question to wiebke. Right now, when the user defines, let's
# say:
# "~ stflife + steco"
# the the CMV of these variables is calculate and replace.
# However, currently only the quality is replaced for these two variables
# in the correlation. It could potentially be that in .data there
# are variables available in me_data which have quality available.
# However, these are not being replaced, only variables which
# we extract from the model_string. Perhaps the best way to do this
# would be to add an additional operator in model_syntax to be able
# to extract also the quality of variables, separately.

# TODO: Redo the vignette with new pkg workflow, which is commented out
