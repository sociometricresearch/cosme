...
ess_model <-
  "# First cmv
   stf = stflife + stfeco + sin(20) * 100
   std(stf_second) = stflife + (stfeco * 100)
   # Second cmv
   ~ ppltrst + trstprl
   # Third cmv
   ~ stflife + ppltrst"
...
## ess4es_complete <- setNames((as.data.frame((replicate(4, sample(1:10, 2e4, replace = TRUE))))), selected_vars)
...
head(create_sscore(parsed_model, ess4es_complete))
